#!/bin/bash
declare -r version="v2.1.2-20201001"
################################################################
...
readonly DEPENDENCIES=(which tar curl sed grep cat awk tput dialog whiptail)
...
function choosedisk()
{
    ...
    do
		...
		modelo=($(fdisk -l | grep -E "(Modelo|Model)"|sed 's/^[:\t]*//'|cut -d':' -f2 | sed 's/^[ \t]*//;s/[ \t]*$//'|sed 's/[[:space:]]\+/_/g'))
